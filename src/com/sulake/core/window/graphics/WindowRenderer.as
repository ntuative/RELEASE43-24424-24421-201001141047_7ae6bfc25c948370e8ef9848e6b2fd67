package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc1_ = var_537.pop() as WindowController;
            if(!_loc1_.disposed)
            {
               _loc3_ = getWindowRendererItem(_loc1_);
               var_288.x = _loc3_.dirty.x;
               var_288.y = _loc3_.dirty.y;
               var_288.width = _loc3_.dirty.width;
               var_288.height = _loc3_.dirty.height;
               renderWindowBranch(_loc1_,_loc3_,var_288);
            }
            _loc4_++;
         }
      }
      ...
      {
         param1.removeEventListener(WindowDisposeEvent.const_626,windowDisposedCallback);
         var _loc2_:WindowRendererItem = var_289[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            _loc2_ = new WindowRendererItem(this,_drawBufferAllocator,var_680);
            var_289[param1] = _loc2_;
            param1.addEventListener(WindowDisposeEvent.const_626,windowDisposedCallback);
         }
      }
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            var_288.x = param2.x - param1.x;
            var_288.y = param2.y - param1.y;
            var_288.width = param2.width;
            var_288.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_30))
            {
               while(param1.testParamFlag(WindowParam.const_30))
               {
                  ...
                  var_288.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            getWindowRendererItem(param1).invalidate(param1,var_288,WindowRedrawFlag.const_229);
            if(var_537.indexOf(param1) == -1)
            {
               var_537.push(param1);
            }
         }
