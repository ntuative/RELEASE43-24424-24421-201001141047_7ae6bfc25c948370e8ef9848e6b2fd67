package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_365 >= var_283.length)
         {
            return "";
         }
         var petColor:IPetColor = var_283[var_365] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_505[var_1323] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_365 = param1.index;
         if(var_365 < 0 || var_365 > var_283.length)
         {
            var_365 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_887.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_165,onTextWindowEvent);
         var_887 = new Map();
         ...
         var_714 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_283 = [];
         var_505 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_714);
               if(_loc11_)
               {
                  var_505 = _loc11_.breeds;
                  var_283 = _loc11_.colors;
                  if(var_283 == null)
                  {
                     var_283 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_505.length)
         {
            _loc12_ = var_505[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_714,_loc12_.id),getRaceLocalizationKey(var_714,_loc12_.id)));
            _loc6_++;
         }
         var_887.add(var_714,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_902,onApproveNameResult);
         _loc5_ = var_887.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_283)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_887.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_365 >= 0 && var_365 < var_283.length)
         {
            _loc7_ = var_283[var_365] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_505[var_1323] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_714,_loc3_.id,_loc2_,AvatarScaleType.const_51);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_308,AvatarAction.const_721);
